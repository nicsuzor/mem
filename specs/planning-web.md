...
created: 2026-03-07
id: pkb-cbc676d1
modified: 2026-04-30T05:04:13.744730680+00:00
status: inbox
tags:
- spec
- planning-web
...
- Consumes: `aops graph` CLI output (`graph.json`, `knowledge-graph.json`), PKB MCP server
- Related: `theme-guide.md`, `batch-graph-operations.md`, [[multi-parent]] §7 (focus_score canonical model)
...
Tasks are ranked by a continuous `focus_score` — the canonical composite signal used by every Planning Web ranking surface. `focus_score` embeds lexicographic severity (SEV4 dominates), priority, downstream weight, urgency (deadline slack + decay), stakeholder waiting, and uncertainty. Component fields (`urgency`, `downstream_weight`, `criticality`) remain visible on the node-detail panel for filter and debug use, but every ranked list, sort order, and "what's hot" view consumes `focus_score` only.

The canonical model is defined in [[multi-parent]] §7. Scores are computed by mem at query time (or emitted on graph build), never stored on the markdown nodes. A score breakdown is available on demand for transparency, so the user can see *why* a node ranks where it does — but the UI never asks the user to choose a ranking signal. There is one signal: `focus_score`.
