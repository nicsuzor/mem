...
- **Orphan Detection**: Disconnected nodes (orphans) can be identified by users and agents through specific graph traversal tools or dedicated commands. Orphan-ness is **type-aware**, reflecting the two structural models the PKB uses:
  ...
  - **Knowledge types** (memory, note, …) live in a *web* of associations expressed as wikilinks. One is an orphan only if it has no valid parent **and** no deliberate structural edge to another node. Body `[[wikilinks]]` are auto-extracted into `link` edges, so a memory carrying one wikilink to a related concept is *not* an orphan. Auto-computed `similar_to` (similarity) edges are excluded — only deliberate links rescue a node.
...
| 17 | `pkb_orphans`         | Graph      | Find orphan nodes (type-aware): actionable types orphan on missing `parent`; knowledge types (memory/note) orphan only with no parent AND no deliberate edge (wikilinks count; `similar_to` excluded). See "Orphan Detection" above.                                                              |
