...
- `/ max(effort_days(x), VOI_EFFORT_NEUTRAL_DAYS)` is the cost normalization (information-gap-ratio form, kb-9230ba76 §5): it prevents a long probe from outranking a short one with the same downstream uncertainty. `VOI_EFFORT_NEUTRAL_DAYS` (3 days) is **both** the default for unestimated work and the floor on the divisor. The two MUST be the same constant: when they differ, the estimate itself moves the score. They previously did — default 3, floor 0.5, and `parse_effort_days` ceils every sub-day estimate to 1 day — so writing `effort: 1h` on a task tripled its VoI with no change to the work (observed live on `brain_8e7dcf7a`: 1437.5 → 4312.5 across an edit that added only that field). With one constant, adding an estimate can only ever *lower* VoI, and only when the estimate genuinely exceeds the neutral point. Effort is self-reported and ungoverned, so optimistic self-report earns nothing while honest largeness is still discounted. A corollary: all estimates at or below 3 days are equivalent for VoI.
