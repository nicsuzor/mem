<script lang="ts">
    ...
    import { computeBaseNodeData, getEdgeVisibilityState, getEdgeLineStyle, getEdgeOpacity, getEdgeWidth } from "../graph/CytoscapeHelpers";
    ...
    function buildElements(nodes: GraphNode[], edges: GraphEdge[], currentFilters: any, currentSettings: any) {
        ...
        edges.forEach((e, idx) => {
            ...
            else if (e.type === 'depends_on') vis = currentFilters.edgeDependencies;
            else if (e.type === 'soft_depends_on') vis = currentFilters.edgeSoftDependencies;
            else if (e.type === 'contributes_to') vis = currentFilters.edgeContributes;
    ...
    $: layoutOptions = {
        ...
        edgeSymDiffLength: (edge: any) => {
            if (edge.data('edgeType') === 'parent') return $viewSettings.colaLinkWeightIntraParent;
