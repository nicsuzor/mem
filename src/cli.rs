...
use mem::{
    document_crud, embeddings, eval, excalidraw, graph, graph_display, graph_store, lint,
    mcp_server, metrics, pkb, task_index, vectordb,
};
...
enum Commands {
    ...
    Graph {
        /// Output format: json, graphml, mcp-index, excalidraw, all
        ...
        /// Focus node ID for ego network layout (used by excalidraw format)
        #[arg(short = 'F', long)]
        focus: Option<String>,
    ...
    match command {
        ...
        Commands::Graph {
            ...
            match format.to_lowercase().as_str() {
                ...
                other => {
                    eprintln!(
                        "Unknown format: {other}. Use: all, json, graphml, mcp-index, excalidraw"
                    );
                    std::process::exit(1);
                }
            }
