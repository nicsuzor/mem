...
fn test_list_tasks_has_superseded_by_filter() {
    ...
    let t0 = &json_tasks[0];
    assert_eq!(
        t0.get("id").and_then(|v| v.as_str()),
        Some("task-open-superseded")
    );
    ...
    let res_empty = server
        .handle_list_tasks(
            &json!({"has_superseded_by": true, "project": "nonexistent", "format": "json"}),
        )
        .unwrap();
    let text_empty = res_empty.content[0].raw.as_text().unwrap().text.as_str();
    assert!(
        text_empty.contains("No tasks found matching filters.")
            || text_empty.contains("\"total\": 0"),
    ...
    let res_filtered_md = server
        ...
        .unwrap();
    let md_text = res_filtered_md.content[0]
        .raw
        .as_text()
        .unwrap()
        .text
    ...
    let res_unfiltered_md = server
        .handle_list_tasks(&json!({"format": "markdown"}))
        .unwrap();
    let unf_md_text = res_unfiltered_md.content[0]
        .raw
        .as_text()
        .unwrap()
        .text
...
fn test_untyped_node_with_id_is_indexed_in_list_tasks_and_emits_parse_warning() {
    ...
    let warnings = get_val
        .get("parse_warnings")
        .and_then(|w| w.as_array())
        .expect("parse_warnings array must exist");
    assert_eq!(warnings.len(), 1);
    assert_eq!(
        warnings[0].get("field").and_then(|f| f.as_str()),
        Some("type")
    );
    assert!(warnings[0]
        .get("message")
...
fn test_task_summary_global_and_project_scoping_regression() {
    ...
    assert_eq!(
        global_blocked, 1,
        "global blocked count matches test graph (t5)"
    );

    // 2. Project-scoped summary for proj-one (3 ready, 0 blocked)
    let p1_res = server
        .handle_task_summary(&json!({"project": "proj-one"}))
        .unwrap();
    ...
    assert_eq!(
        p1_ready, p1_list_total,
        "task_summary(proj-one).ready == list_tasks(proj-one, ready).total"
    );

    // Check alias resolution: ProjectOne alias matches proj-one
    let p1_alias_res = server
        .handle_task_summary(&json!({"project": "ProjectOne"}))
        .unwrap();
    ...
    let p2_res = server
        .handle_task_summary(&json!({"project": "proj-two"}))
        .unwrap();
    ...
    assert_eq!(
        p2_ready, p2_list_total,
        "task_summary(proj-two).ready == list_tasks(proj-two, ready).total"
    );

    // Assert that the two projects have differing counts (3 vs 1)
    assert_ne!(
        p1_ready, p2_ready,
        "proj-one and proj-two have differing ready counts"
    );
    assert_ne!(
        p1_val, p2_val,
    ...
    let nonexist_res = server
        .handle_task_summary(&json!({"project": "does-not-exist"}))
        .unwrap();
...
fn test_task_summary_unsupported_parameters_rejected() {
    ...
    assert!(
        err.message
            .contains("Unsupported parameter(s) for task_summary: status"),
    ...
    assert!(
        err2.message
            .contains("Unsupported parameter(s) for task_summary: foo"),
        "Error message must name unsupported parameter: {}",
        err2.message
    );
...
fn test_list_tasks_array_valued_status_filter() {
    ...
    assert!(
        !tasks_multi.is_empty(),
        "Should return tasks in ready or in_progress"
    );
    ...
    assert!(
        !tasks_done.is_empty(),
        "Explicit array status done must surface done tasks"
    );
    for t in &tasks_done {
...
fn test_list_tasks_invalid_status_types_and_values_rejected() {
    ...
    assert!(err_arr_elem
        .message
        .contains("Invalid status filter element: expected string"));
    ...
    assert!(err_val
        .message
        .contains("Invalid status filter \"not_a_real_status\""));
    ...
    assert!(err_arr_val
        .message
        .contains("Invalid status filter \"bogus_status\""));
...
fn test_list_tasks_format_ascii_tree_and_nested_flag() {
    ...
    let res_nested = server.handle_list_tasks(&json!({"nested": true})).unwrap();
    ...
    let res_view = server.handle_list_tasks(&json!({"view": "tree"})).unwrap();
...
fn test_list_tasks_nested_json_brief_metadata() {
    ...
    let val: serde_json::Value =
        serde_json::from_str(text).expect("nested json must be valid JSON");
    ...
    assert_eq!(
        a1_task.get("title").and_then(|t| t.as_str()),
        Some("Alpha Task 1")
    );
    assert_eq!(
        a1_task.get("status").and_then(|s| s.as_str()),
        Some("ready")
    );
    ...
    assert!(
        a1_task.get("signals").is_none(),
        "brief metadata must not contain heavy signals object"
    );
    assert!(
        a1_task.get("body").is_none(),
        "brief metadata must not contain document body"
    );
    ...
    let res_alias = server
        ...
        .unwrap();
    let val_alias: serde_json::Value =
        serde_json::from_str(res_alias.content[0].raw.as_text().unwrap().text.as_str())
...
fn test_nested_tasks_tool_ascii_tree_and_json_options() {
    ...
    let res_default = server.handle_nested_tasks(&json!({})).unwrap();
    let text_default = res_default.content[0].raw.as_text().unwrap().text.as_str();
    assert!(
        text_default.contains("▌"),
        "nested_tasks default must be ascii tree with ▌ context indicators"
    );
    assert!(text_default.contains("├── ") || text_default.contains("└── "));
    assert!(
        !text_default.contains("\x1b["),
        "nested_tasks ascii tree must not contain ANSI escapes"
    );
    ...
    let val_json: serde_json::Value =
        serde_json::from_str(text_json).expect("nested_tasks json must be valid JSON");
    ...
    let val_filtered: serde_json::Value =
        serde_json::from_str(res_filtered.content[0].raw.as_text().unwrap().text.as_str()).unwrap();
    let tasks_filtered = val_filtered["tasks"].as_array().unwrap();
    assert_eq!(
        tasks_filtered.len(),
        1,
        "only proj-alpha container should be returned"
    );
...
#[test]
fn test_nested_tasks_null_or_non_object_args_defaults_to_ascii_tree() {
    let server = build_test_server();

    // Calling nested_tasks with null or non-object args must NOT fall back to flat markdown table.
    // It must enforce nested=true and default format="ascii_tree".
    let res_null = server.handle_nested_tasks(&json!(null)).unwrap();
    let text_null = res_null.content[0].raw.as_text().unwrap().text.as_str();
    assert!(
        text_null.contains("▌"),
        "nested_tasks with null args must output ascii tree with ▌ indicators: {text_null}"
    );
    assert!(
        text_null.contains("├── ") || text_null.contains("└── "),
        "nested_tasks with null args must contain tree connectors: {text_null}"
    );
    assert!(
        !text_null.contains("| PRI |") && !text_null.contains("| :---"),
        "nested_tasks with null args must not fall back to flat markdown table: {text_null}"
    );

    let res_str = server
        .handle_nested_tasks(&json!("invalid_scalar"))
        .unwrap();
    let text_str = res_str.content[0].raw.as_text().unwrap().text.as_str();
    assert!(text_str.contains("▌"));
}

#[test]
fn test_list_tasks_json_tree_alias_and_schema() {
    let server = build_test_server();

    // 1. Runtime: format="json_tree" returns nested JSON
    let res = server
        .handle_list_tasks(&json!({"format": "json_tree"}))
        .unwrap();
    let text = res.content[0].raw.as_text().unwrap().text.as_str();
    let val: serde_json::Value =
        serde_json::from_str(text).expect("json_tree must return valid JSON");
    assert!(val.get("tasks").is_some());
    assert!(val.get("total").is_some());

    // 2. Schema honesty: list_tasks schema enum must include "json_tree"
    let tools = PkbSearchServer::get_all_tools();
    let list_tool = tools
        .iter()
        .find(|t| t.name.as_ref() == "list_tasks")
        .expect("list_tasks must exist");
    let schema = serde_json::to_value(&list_tool.input_schema).unwrap();
    let format_prop = schema
        .pointer("/properties/format/enum")
        .and_then(|v| v.as_array())
        .expect("format enum must exist in list_tasks");
    let enum_values: Vec<&str> = format_prop.iter().filter_map(|v| v.as_str()).collect();
    assert!(
        enum_values.contains(&"json_tree"),
        "list_tasks schema format enum must include 'json_tree', got: {enum_values:?}"
    );
}

#[test]
fn test_nested_tasks_schema_format_enum_excludes_markdown() {
    let tools = PkbSearchServer::get_all_tools();
    let nested_tool = tools
        .iter()
        .find(|t| t.name.as_ref() == "nested_tasks")
        .expect("nested_tasks must exist");
    let schema = serde_json::to_value(&nested_tool.input_schema).unwrap();
    let format_prop = schema
        .pointer("/properties/format/enum")
        .and_then(|v| v.as_array())
        .expect("format enum must exist in nested_tasks");
    let enum_values: Vec<&str> = format_prop.iter().filter_map(|v| v.as_str()).collect();

    assert!(
        !enum_values.contains(&"markdown"),
        "nested_tasks schema must not list misleading 'markdown' format enum: {enum_values:?}"
    );
    assert!(
        enum_values.contains(&"ascii_tree"),
        "nested_tasks schema must contain 'ascii_tree': {enum_values:?}"
    );
    assert!(
        enum_values.contains(&"json"),
        "nested_tasks schema must contain 'json': {enum_values:?}"
    );
    assert!(
        enum_values.contains(&"nested_json"),
        "nested_tasks schema must contain 'nested_json': {enum_values:?}"
    );
}

#[test]
fn test_nested_tasks_fields_filtering() {
    let server = build_test_server();
    ...
    let tasks = val.get("tasks").and_then(|v| v.as_array()).unwrap();
    ...
    for node in tasks {
        ...
        assert!(
            ...
            "unrequested field 'status' should be filtered out"
        );
        assert!(
            !obj.contains_key("intent"),
            "unrequested field 'intent' should be filtered out"
        );
    }
}
