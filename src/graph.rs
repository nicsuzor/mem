...
/// structural parent (linked to via `contributes_to`, never `parent`).
pub fn is_strategic_target(node_type: Option<&str>) -> bool {
    matches!(node_type, Some(t) if STRATEGIC_TARGET_TYPES.contains(&t))
}
